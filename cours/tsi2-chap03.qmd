...
::: {.example title="(Diagonalisation d'une matrice $2 \times 2$)"}
...
La matrice de passage est $P = \begin{pmatrix} 1 & 1 \\ 1 & -1 \end{pmatrix}$ et :
$$\mathrm{Mat}_{\mathscr{B}'}(u) = \begin{pmatrix} \frac{1}{2} & 0 \\ 0 & \frac{3}{2} \end{pmatrix}$$
...
::: {.example title="(Calcul de puissances de matrices)"}

Si $A$ est diagonalisable avec $A = PDP^{-1}$, alors :
$$A^n = PD^nP^{-1}$$
où $D^n = \mathrm{Diag}(\lambda_1^n, \ldots, \lambda_r^n)$.

:::

::: {.example title="(Résolution de systèmes différentiels)"}
...
::: {.example title="(Suites récurrentes linéaires)"}
...
# Document en PDF {-}
📝 [**Chapitre 03 : Réduction des endomorphismes et des matrices carrées**](PDF/tsi2-chap03.pdf)

# Voir TD {-}
✍️ [**TD 03 : Réduction des endomorphismes et des matrices carrées**](../td/tsi2-chap03.qmd)

## Navigation {-}

::: {.chap-nav}

[Chapitre 02 : Déterminants](tsi2-chap02.qmd){.prev}
[Menu TSI](tsi2.qmd){.home}
[Chapitre 04 : Fonctions vectorielles d'une variable réelle, arcs paramétrés](tsi2-chap04.qmd){.next}
