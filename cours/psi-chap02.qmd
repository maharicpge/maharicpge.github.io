---
title: "Cours – Réduction des endomorphismes et des matrices carrées"
...
**PSI (2e année) — Chapitre 02.** Éléments propres, polynôme caractéristique, endomorphismes et matrices diagonalisables, polynômes annulateurs, endomorphismes et matrices trigonalisables.
...
Soient $u \in \mathcal{L}(E)$ et $x_0 \in E \setminus \{0_E\}$. On suppose que la droite vectorielle $D = \mathbb{K} \cdot x_0$ est stable par $u$. Alors il existe un scalaire $\lambda \in \mathbb{K}$ tel que :
...
Soit $u$ la transformation du plan $\mathbb{R}^2$ définie par $u(x,y) = (x - y/2, -x/2 + y)$.

La matrice de $u$ dans la base canonique est :
$$\mathrm{Mat}_{\mathscr{B}}(u) = \begin{pmatrix} 1 & -\frac{1}{2} \\ -\frac{1}{2} & 1 \end{pmatrix}$$

Le polynôme caractéristique est :
$$\chi_u(X) = (1-X)^2 - \frac{1}{4} = \left(X - \frac{3}{2}\right)\left(X - \frac{1}{2}\right)$$

Le polynôme caractéristique est scindé à racines simples, donc $u$ est diagonalisable.

Les sous-espaces propres sont :
$$E_{1/2} = \mathrm{Vect}((1,1)), \quad E_{3/2} = \mathrm{Vect}((1,-1))$$

:::

# Polynômes d'endomorphismes et de matrices carrées

## Morphisme d'évaluation et idéal annulateur
...
::: {.proposition title="(Famille de projecteurs associés à une décomposition)"}
...
::: {.theorem title="(Caractérisation de la diagonalisabilité par le polynôme minimal)"}
...
Soit $M_1 = \begin{pmatrix} 1 & 4 & -2 \\ 0 & 6 & -3 \\ -1 & 4 & 0 \end{pmatrix}$.

**Étape 1** : Calcul du polynôme caractéristique :
$$\chi_{M_1}(X) = (X-2)^2(X-3)$$

**Étape 2** : Recherche des sous-espaces propres :
$$E_2 = \mathrm{Vect}\begin{pmatrix} -2 \\ 0 \\ -1 \end{pmatrix}, \quad E_3 = \mathrm{Vect}\begin{pmatrix} 1 \\ 1 \\ 1 \end{pmatrix}$$

**Étape 3** : Comme $\dim(E_2) = 1 < 2 = \mu_2$, la matrice $M_1$ n'est pas diagonalisable. Cependant, elle est trigonalisable car $\chi_{M_1}$ est scindé.
...
# Document en PDF {-}
📝 [**Chapitre 02 : Réduction des endomorphismes et des matrices carrées**](PDF/psi-chap02.pdf)

# Voir TD {-}
✍️ [**TD 02 : Réduction des endomorphismes et des matrices carrées**](../td/psi-chap02.qmd)

## Navigation {-}

::: {.chap-nav}

[Chapitre 01 : Rappels et compléments sur les espaces vectoriels, les endomorphismes et les matrices](psi-chap01.qmd){.prev}
[Menu PSI](psi.qmd){.home}
[Chapitre 03 : Espaces préhilbertiens réels](psi-chap03.qmd){.next}
